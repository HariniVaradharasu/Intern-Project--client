...
const Purchase = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await axios.get('http://localhost:7000/api/purchasers/getItems');
  ...
  const handleAddItem = async () => {
    ...
    try {
      if (editingItem) {
        // Update item
        const response = await axios.put(`http://localhost:7000/api/purchasers/updateItem/${editingItem._id}`,newItem
        );
        setTableData(tableData.map((i) =>i._id === editingItem._id ? response.data : i));
        setEditingItem(null);
      } else {
        // Add new item
        const response = await axios.post('http://localhost:7000/api/purchasers/createItem', newItem);
  ...
  const handleDeleteItem = async (itemId) => {
    try {
      await axios.delete(`http://localhost:7000/api/purchasers/deleteItem/${itemId}`);
